...
echo ">>>>>>> "
echo -n ">>>>>>> "
echo "INSTALLATION WITH 'R CMD INSTALL --preclean --library=$R_LIBS $srcpkg_filepath'"
echo ">>>>>>> "
echo ""
$R_CMD CMD INSTALL --preclean --library="$R_LIBS" "$srcpkg_filepath"
echo ""
echo ""
...
if [ -d "$so_path" ]; then
    for arch in $TARGET_ARCHS; do
        ...
        if [ ! -d "$arch_so_path" ]; then
            ...
            echo ">>>>>>> "
            echo -n ">>>>>>> "
            echo "INSTALLATION OF $arch LIBS WITH 'R_ARCH=/$arch R CMD INSTALL --preclean --library=$R_LIBS --libs-only $srcpkg_filepath'"
            echo ">>>>>>> "
            echo ""
